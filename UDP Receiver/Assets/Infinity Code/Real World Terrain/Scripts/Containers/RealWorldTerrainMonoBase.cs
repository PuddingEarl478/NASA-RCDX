...
namespace InfinityCode.RealWorldTerrain
{
    ...
    public abstract class RealWorldTerrainMonoBase : MonoBehaviour
    {
        ...
        public double GetAltitudeByCoordinates(double lng, double lat)
        {
            ...
            {
                float px = offset.x / b.size.x * prefs.heightmapResolution;
                float py = offset.z / b.size.z * prefs.heightmapResolution;
                RealWorldTerrainItem currentItem = GetItemByWorldPosition(worldPosition);
                return currentItem.terrainData.GetHeight((int)px, (int)py) / currentItem.terrainData.size.y * (currentItem.maxElevation - currentItem.minElevation);
            }
        ...
        public bool GetCoordinatesByWorldPosition(Vector3 worldPosition, out double longitude, out double latitude, out double altitude)
        {
            ...
            if (!b.Contains(worldPosition)) return false;
            ...
            {
                RealWorldTerrainItem currentItem = GetItemByWorldPosition(worldPosition);
                float px = (offset.x - currentItem.transform.localPosition.x) / currentItem.bounds.size.x * prefs.heightmapResolution;
                float py = (offset.z - currentItem.transform.localPosition.z) / currentItem.bounds.size.z * prefs.heightmapResolution;
                altitude = currentItem.terrainData.GetHeight((int)px, (int)py) / currentItem.terrainData.size.y * (currentItem.maxElevation - currentItem.minElevation) + currentItem.minElevation;
            }
            else
            {
